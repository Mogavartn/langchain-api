...
class MemoryManager:
    ...
    @staticmethod
    def trim_memory(memory: ConversationBufferMemory, max_messages: int = 25):
...
async def memory_status():
    ...
    try:
        ...
        return {
            ...
            "optimization": "Auto-trim to 25 messages"
        }
    except Exception as e:
...
async def health_check():
    ...
    return {
        ...
        "active_sessions": len(memory_store),
        "memory_type": "ConversationBufferMemory (Optimized)",
        "memory_optimization": "Auto-trim to 25 messages",
        "improvements": [
            "Fixed conversation context management",
            "Enhanced memory analysis",
            "Corrected priority rules logic",
            "Better contextual response detection",
            "Improved CPF delay handling",
            "Enhanced payment context processing",
            "Fixed ambassadeur context management",
            "Robust pattern detection and memory debugging"
        ]
    }
...
class ConversationContextManager:
    ...
    def analyze_conversation_context(user_message: str, memory: ConversationBufferMemory) -> Dict[str, Any]:
        """Analyse le contexte de la conversation pour adapter la réponse - VERSION CORRIGÉE"""
        
        # Récupérer l'historique
        history = memory.chat_memory.messages
        message_count = len(history)
        
        # LOGS DEBUG MÉMOIRE AMÉLIORÉS
        logger.info(f"=== ANALYSE CONTEXTE MÉMOIRE ===")
        logger.info(f"Nombre total de messages: {message_count}")
        logger.info(f"Message utilisateur actuel: '{user_message}'")
        
        if message_count > 0:
            logger.info("=== HISTORIQUE DES 10 DERNIERS MESSAGES ===")
            for i, msg in enumerate(history[-10:]):
                msg_type = getattr(msg, 'type', 'unknown')
                content = str(msg.content)[:200] + "..." if len(str(msg.content)) > 200 else str(msg.content)
                logger.info(f"Message {i}: Type={msg_type}, Content='{content}'")
        
        # Initialiser les variables de contexte
        conversation_state = "initial"
        awaiting_specific_response = False
        last_bot_question = None
        is_follow_up = message_count > 0
        
        # Variables pour détecter les contextes spécifiques
        awaiting_cpf_info = False
        awaiting_financing_info = False
        awaiting_formation_date = False
        previous_topic = None
        
        # ANALYSE DE L'HISTORIQUE POUR DÉTECTER LE CONTEXTE
        if message_count > 0:
            # Analyser les 5 derniers messages pour détecter le contexte
            ...
            for i, msg in enumerate(reversed(recent_messages)):
                ...
                logger.info(f"Analyse message {i}: {content[:100]}...")
                
                # DÉTECTION QUESTION DE FINANCEMENT (bot demande le type de financement)
                if ("comment la formation a été financée" in content or 
                    "comment la formation a-t-elle été financée" in content):
                    awaiting_financing_info = True
                    conversation_state = "awaiting_financing_type"
                    last_bot_question = "financing_type"
                    logger.info("🔍 CONTEXTE DÉTECTÉ: En attente du type de financement")
                    break
                    
                # DÉTECTION QUESTION DE DÉLAI (bot demande quand s'est terminée la formation)
                if ("environ quand la formation s'est-elle terminée" in content or
                    "quand la formation s'est terminée" in content):
                    awaiting_formation_date = True
                    conversation_state = "awaiting_formation_date"
                    last_bot_question = "formation_date"
                    logger.info("🔍 CONTEXTE DÉTECTÉ: En attente de la date de formation")
                    break
                    
                # DÉTECTION QUESTION CPF BLOQUÉ
                if ("dossier cpf faisait partie des quelques cas bloqués" in content or
                    "cas bloqués par la caisse des dépôts" in content):
                    awaiting_cpf_info = True
                    conversation_state = "awaiting_cpf_confirmation"
                    last_bot_question = "cpf_blocked_confirmation"
                    logger.info("🔍 CONTEXTE DÉTECTÉ: En attente de confirmation CPF bloqué")
                    break
                    
                # DÉTECTION QUESTION AMBASSADEUR
                if ("tu veux en savoir plus sur comment devenir ambassadeur" in content or
                    "tu veux devenir ambassadeur" in content):
                    conversation_state = "awaiting_ambassador_interest"
                    last_bot_question = "ambassador_interest"
                    logger.info("🔍 CONTEXTE DÉTECTÉ: En attente de l'intérêt pour devenir ambassadeur")
                    break
        
        # ANALYSE DU MESSAGE UTILISATEUR DANS LE CONTEXTE
        user_message_lower = user_message.lower()
        is_contextual_response = False
        
        if awaiting_financing_info or awaiting_formation_date or awaiting_cpf_info:
            awaiting_specific_response = True
            
            # DÉTECTION RÉPONSE TYPE DE FINANCEMENT
            if awaiting_financing_info:
                financing_keywords = ["cpf", "opco", "entreprise", "direct", "particulier", "patron"]
                if any(keyword in user_message_lower for keyword in financing_keywords):
                    is_contextual_response = True
                    logger.info("✅ RÉPONSE CONTEXTUELLE: Type de financement détecté")
                    
            # DÉTECTION RÉPONSE DÉLAI
            if awaiting_formation_date:
                delay_patterns = ["mois", "il y a", "ça fait", "ca fait", "depuis", "terminé", "fini"]
                if any(pattern in user_message_lower for pattern in delay_patterns):
                    is_contextual_response = True
                    logger.info("✅ RÉPONSE CONTEXTUELLE: Délai détecté")
                    
            # DÉTECTION CONFIRMATION CPF
            if awaiting_cpf_info:
                confirmation_words = ["oui", "yes", "ok", "effectivement", "c'est ça", "exact"]
                negation_words = ["non", "no", "pas", "jamais"]
                if any(word in user_message_lower for word in confirmation_words):
                    is_contextual_response = True
                    logger.info("✅ RÉPONSE CONTEXTUELLE: Confirmation CPF détectée")
                elif any(word in user_message_lower for word in negation_words):
                    is_contextual_response = True
                    logger.info("✅ RÉPONSE CONTEXTUELLE: Négation CPF détectée")
        
        # RÉSULTAT FINAL
        context_result = {
            "message_count": message_count,
            "conversation_state": conversation_state,
            "awaiting_specific_response": awaiting_specific_response,
            "awaiting_financing_info": awaiting_financing_info,
            "awaiting_formation_date": awaiting_formation_date,
            "awaiting_cpf_info": awaiting_cpf_info,
            "last_bot_question": last_bot_question,
            "is_contextual_response": is_contextual_response,
        ...
        logger.info(f"=== CONTEXTE FINAL ===")
        logger.info(f"État: {conversation_state}")
        logger.info(f"En attente de réponse spécifique: {awaiting_specific_response}")
        logger.info(f"Réponse contextuelle: {is_contextual_response}")
        logger.info(f"Dernière question bot: {last_bot_question}")
...
class MessageProcessor:
    ...
    def detect_priority_rules(user_message: str, matched_bloc_response: str, conversation_context: Dict[str, Any]) -> Dict[str, Any]:
        """Applique les règles de priorité avec prise en compte du contexte - VERSION CORRIGÉE MÉMOIRE"""
        
        message_lower = user_message.lower()
        
        # PRIORITÉ ABSOLUE: TRAITEMENT DES RÉPONSES CONTEXTUELLES
        logger.info(f"=== ANALYSE PRIORITÉS CONTEXTUELLES ===")
        logger.info(f"Message: '{user_message}'")
        logger.info(f"Contexte: {conversation_context}")
        
        # ÉTAPE 1: RÉPONSE À UNE QUESTION DE FINANCEMENT
        if conversation_context.get("awaiting_financing_info"):
            logger.info("🎯 PRIORITÉ: Réponse à la question de financement")
            
            financing_type = PaymentContextProcessor.extract_financing_type(user_message)
            if financing_type:
                # Ne PAS utiliser le bloc CPF indisponible, demander le délai
                return {
                    "use_matched_bloc": False,
                    "priority_detected": "FINANCING_TYPE_RECEIVED",
                    "response": "Et environ quand la formation s'est-elle terminée ? 📅",
                    "context": conversation_context,
                    "financing_info": financing_type
                }
        
        # ÉTAPE 2: RÉPONSE À UNE QUESTION DE DÉLAI (FINANCEMENT + DÉLAI)
        if conversation_context.get("awaiting_formation_date"):
            logger.info("🎯 PRIORITÉ: Réponse à la question de délai")
...
Sinon, je fais remonter ta demande à notre équipe pour vérification ✅""",
        ...
        # ÉTAPE 3: RÉPONSE À UNE QUESTION CPF BLOQUÉ
        if conversation_context.get("awaiting_cpf_info"):
            logger.info("🎯 PRIORITÉ: Réponse à la confirmation CPF bloqué")
            
            user_lower = user_message.lower()
            
            # Si l'utilisateur confirme qu'il était informé du blocage
            if any(word in user_lower for word in ['oui', 'yes', 'informé', 'dit', 'déjà', 'je sais', 'effectivement']):
                return {
                    "use_matched_bloc": False,
                    "priority_detected": "CPF_BLOQUE_CONFIRME",
                    "response": """On comprend parfaitement ta frustration. Ce dossier fait partie des quelques cas (moins de 50 sur plus de 2500) bloqués depuis la réforme CPF de février 2025. Même nous n'avons pas été payés. Le blocage est purement administratif, et les délais sont impossibles à prévoir. On te tiendra informé dès qu'on a du nouveau. Inutile de relancer entre-temps 🙏
...
🔄 Escalade: AGENT ADMIN""",
        ...
        # ÉTAPE 5: Gestion spécifique AMBASSADEUR
        ...
        # ÉTAPE 6: Détection problème paiement formation
        payment_keywords = [
...
🕐 Horaires : Lundi-Vendredi, 9h-17h""",
        ...
        # ÉTAPE 7: Bloc matché (si pas de problème de paiement détecté)
        ...
        # ÉTAPE 8: Messages de suivi généraux
        if conversation_context["is_follow_up"] and conversation_context["message_count"] > 0:
            return {
                "use_matched_bloc": False,
...
async def process_message(request: Request):
    ...
    try:
        ...
        MemoryManager.trim_memory(memory, max_messages=25)
        ...
        elif priority_result.get("priority_detected") == "FINANCING_TYPE_RECEIVED":
            final_response = priority_result["response"]
            response_type = "financing_type_received"
            escalade_required = False
...
On te tiendra informé dès que possible ✅"""
        ...
        response_data = {
            ...
            "use_ai": priority_result.get("use_ai", False)
        }
